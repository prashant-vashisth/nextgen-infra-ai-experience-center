...
const LOG_TYPE_COLOR = { thinking: 'text-gray-400', tool_call: 'text-humana-teal', tool_result: 'text-humana-green', action: 'text-blue-400', complete: 'text-white', progress: 'text-amber-400', waiting: 'text-yellow-400', created: 'text-humana-green', summary: 'text-white', validated: 'text-humana-green' }
...
export default function CVITWorkflowAgent() {
  ...
  const [stepStates, setStepStates] = useState({})      // stepId → 'active' | 'done' | 'error'
  const [logs, setLogs] = useState([])
  const [findings, setFindings] = useState([])
  const [selectedCluster, setSelectedCluster] = useState(CLUSTERS[0])
  const [humanPending, setHumanPending] = useState(null)  // { action, data }
  const [progress, setProgress] = useState(0)
  const [artifacts, setArtifacts] = useState({})          // changeTicket, pr, incident, kb
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <div className="flex items-center gap-2 text-xs text-gray-500 mb-1">
              ...
              CVIT Orchestrator · LangGraph + Groq Tool Calling · Azure AKS
            </div>
            <h1 className="text-xl font-bold text-white">Container Vulnerability Intelligent Remediation (CVIT)</h1>
            <p className="text-sm text-gray-400 mt-0.5">10-step multi-agent workflow: Detect → Enrich → Approve → Remediate → Validate → KB</p>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            <select value={selectedCluster} onChange={e => setSelectedCluster(e.target.value)}
              className="bg-gray-800 border border-gray-700 text-white text-xs px-3 py-2 rounded-lg focus:outline-none">
      ...
      <div className="flex h-[calc(100vh-130px)]">
        ...
        <div className="w-52 bg-gray-900 border-r border-gray-800 flex flex-col overflow-y-auto shrink-0">
          <div className="px-3 pt-4 pb-2 text-xs font-bold text-gray-500 uppercase tracking-widest">Workflow Steps</div>
          {STEPS.map(step => {
            const state = stepStates[step.id]
            const isCurrent = currentStep === step.index
            const Icon = step.icon
            return (
              <div key={step.id} className={`flex items-start gap-2 px-3 py-2.5 mx-1.5 rounded-lg mb-0.5 transition-all ${isCurrent ? 'bg-humana-green/15 border border-humana-green/30' : ''}`}>
                <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${
                  state === 'done' ? 'bg-humana-green' : state === 'active' || isCurrent ? 'bg-humana-green/30 border-2 border-humana-green' : state === 'error' ? 'bg-red-600' : 'bg-gray-700'
                }`}>
                  {state === 'done' ? <CheckCircle2 size={10} className="text-white" /> :
                   ...
                   <span className="text-xs text-gray-500 font-bold">{step.index}</span>}
                </div>
                <div>
                  ...
                  {step.human && (
                    <span className="text-xs text-amber-500 font-semibold">👤 Human-in-loop</span>
                  )}
                </div>
              </div>
            )
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {findings.length > 0 && (
            <div className="border-b border-gray-800 bg-gray-900 px-4 py-3">
              <div className="text-xs font-bold text-gray-400 mb-2 uppercase">CVITs Detected — {findings.length} vulnerabilities</div>
              <div className="flex flex-wrap gap-2">
                {findings.map(f => (
                  <div key={f.id} className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-1.5 border border-gray-700">
                    <span className={`text-xs font-black px-1.5 py-0.5 rounded ${SEV_BADGE[f.severity] || 'bg-gray-600 text-white'}`}>{f.severity}</span>
                    <span className="font-mono text-xs text-gray-300 font-semibold">{f.id}</span>
          ...
          {progress > 0 && progress < 100 && (
            <div className="border-b border-gray-800 bg-gray-900 px-4 py-3">
              <div className="flex items-center justify-between text-xs mb-1.5">
                <span className="text-gray-400 font-semibold">Remediation Progress — Agent monitoring live</span>
          ...
          {(artifacts.change || artifacts.incident || artifacts.pr || artifacts.kb) && (
            <div className="border-b border-gray-800 bg-gray-900 px-4 py-3 flex flex-wrap gap-3">
              {artifacts.change && (
                <div className="flex items-center gap-2 bg-amber-900/30 border border-amber-700/40 rounded-lg px-3 py-1.5 text-xs">
                  <FileText size={12} className="text-amber-400" />
                  <span className="text-amber-300 font-semibold">{artifacts.change.number}</span>
                  <span className="text-gray-500">Change Request</span>
                  <span className={`text-xs font-semibold ${artifacts.change.source?.includes('live') ? 'text-humana-green' : 'text-gray-500'}`}>
                    {artifacts.change.source?.includes('live') ? '● Live' : '○ Demo'}
                  </span>
                </div>
              )}
              {artifacts.incident && (
                <div className="flex items-center gap-2 bg-blue-900/30 border border-blue-700/40 rounded-lg px-3 py-1.5 text-xs">
                  <Shield size={12} className="text-blue-400" />
                  <span className="text-blue-300 font-semibold">{artifacts.incident.number}</span>
                  <span className="text-gray-500">ServiceNow Incident</span>
                  <span className={`text-xs font-semibold ${artifacts.incident.source?.includes('live') ? 'text-humana-green' : 'text-gray-500'}`}>
              ...
              {artifacts.pr && (
                <a href={artifacts.pr.url !== '#' ? artifacts.pr.url : undefined} target="_blank" rel="noreferrer"
                  className="flex items-center gap-2 bg-purple-900/30 border border-purple-700/40 rounded-lg px-3 py-1.5 text-xs hover:bg-purple-900/50 transition-colors">
                  <GitPullRequest size={12} className="text-purple-400" />
                  <span className="text-purple-300 font-semibold">PR #{artifacts.pr.number}</span>
                  <span className="text-gray-500">GitHub PR</span>
                  <span className={`text-xs font-semibold ${artifacts.pr.source === 'github_live' ? 'text-humana-green' : 'text-gray-500'}`}>
                    {artifacts.pr.source === 'github_live' ? '● Live' : '○ Demo'}
                  </span>
                </a>
              )}
              {artifacts.kb && (
                <div className="flex items-center gap-2 bg-humana-green/10 border border-humana-green/30 rounded-lg px-3 py-1.5 text-xs">
            ...
            {logs.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-600 gap-4">
                ...
                <div className="text-center">
                  <div className="text-gray-400 font-semibold mb-1">CVIT Multi-Agent Orchestrator</div>
                  <div className="text-gray-600">Select a cluster and click "Launch CVIT Workflow" to start</div>
                  <div className="text-gray-700 mt-2">LangGraph + Groq Tool Calling + Azure + ServiceNow + GitHub</div>
                </div>
              </div>
            ) : (
              <div className="space-y-0.5">
                {logs.map((entry, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }}
                    ...
                    <span className="text-gray-700 shrink-0 w-20 text-right">{new Date(entry.ts).toLocaleTimeString()}</span>
                    <span className={`font-bold shrink-0 w-28 truncate ${
                      entry.agent === 'ScannerAgent' ? 'text-red-400' :
                      entry.agent === 'EnrichmentAgent' ? 'text-amber-400' :
                      entry.agent === 'WorkPackageAgent' ? 'text-purple-400' :
                      entry.agent === 'MonitorAgent' ? 'text-blue-400' :
                      entry.agent === 'KBAgent' ? 'text-humana-green' :
                      'text-humana-teal'
                    }`}>[{entry.agent}]</span>
                    <span className={`shrink-0 w-20 ${LOG_TYPE_COLOR[entry.type] || 'text-gray-400'}`}>{entry.type}</span>
        ...
        <div className="w-72 bg-gray-900 border-l border-gray-800 flex flex-col overflow-y-auto shrink-0">
          <div className="px-4 pt-4 pb-2 text-xs font-bold text-gray-500 uppercase tracking-widest">Human-in-Loop</div>

          <AnimatePresence>
            {humanPending?.action === 'approve' && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                className="mx-3 mb-3 bg-amber-900/30 border border-amber-600/50 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-3 h-3 rounded-full bg-amber-500 animate-pulse" />
                  <span className="text-amber-400 font-bold text-sm">Approval Required</span>
                </div>
                <div className="text-xs text-gray-300 mb-3 leading-relaxed">
                  Change request <span className="font-mono text-amber-300">{humanPending.changeTicket?.number}</span> created in ServiceNow.
                  Risk level: <span className={`font-bold ${humanPending.cve?.cvss >= 8 ? 'text-red-400' : 'text-amber-400'}`}>
                    {humanPending.cve?.cvss >= 8 ? 'HIGH' : 'MEDIUM'} (CVSS {humanPending.cve?.cvss || humanPending.cve?.cvss_score})
                  </span>
                </div>
                <div className="space-y-2 mb-3">
                  <div className="text-xs text-gray-400">CVE: <span className="text-white font-mono">{humanPending.cve?.id || humanPending.cve?.cve_id}</span></div>
                  <div className="text-xs text-gray-400">Component: <span className="text-white">{humanPending.cve?.component} → {humanPending.cve?.patchVersion}</span></div>
                  <div className="text-xs text-gray-400">Namespace: <span className="text-humana-teal">{humanPending.cve?.namespace}</span></div>
                </div>
                <div className="mb-3">
                  ...
                  <input value={approverName} onChange={e => setApproverName(e.target.value)}
                    className="w-full bg-gray-800 border border-gray-700 text-white text-xs px-2 py-1.5 rounded focus:outline-none focus:border-humana-green" />
                </div>
                <div className="flex gap-2">
                  ...
                  <button onClick={handleReject} className="flex-1 flex items-center justify-center gap-1.5 bg-red-700 hover:bg-red-800 text-white text-xs font-bold py-2 rounded-lg transition-colors">
            ...
            {humanPending?.action === 'execute' && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                className="mx-3 mb-3 bg-blue-900/30 border border-blue-600/50 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-3 h-3 rounded-full bg-blue-500 animate-pulse" />
                  <span className="text-blue-400 font-bold text-sm">Execution Confirmation</span>
                </div>
                <div className="text-xs text-gray-300 mb-3 leading-relaxed">
                ...
                <div className="space-y-1.5 mb-3 text-xs">
                  {humanPending.pr && (
                    <div className="flex items-center gap-2">
                      <GitPullRequest size={11} className="text-purple-400" />
                      <span className="text-gray-400">PR #{humanPending.pr.number} created</span>
                      <span className={`${humanPending.pr.source === 'github_live' ? 'text-humana-green' : 'text-gray-600'}`}>
                  ...
                  {humanPending.incident && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-400">{humanPending.incident.number} assigned</span>
                      <span className={`${humanPending.incident.source?.includes('live') ? 'text-humana-green' : 'text-gray-600'}`}>
          ...
          {isCompleted && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              className="mx-3 mb-3 bg-humana-green/10 border border-humana-green/30 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-3">
              ...
              <div className="space-y-1.5 text-xs">
                ...
                {artifacts.kb && <div className="text-gray-400">📚 KB: <span className="font-mono text-white">{artifacts.kb.number}</span></div>}
              </div>
            </motion.div>
          )}
          ...
          <div className="px-4 pt-2">
            <div className="text-xs font-bold text-gray-600 mb-2 uppercase">Agent Framework</div>
            <div className="space-y-1.5 text-xs text-gray-500">
              {[
                ...
                <div key={item.label} className="flex items-center justify-between">
                  ...
                  <span className="text-gray-400 font-mono text-xs">{item.value}</span>
                </div>
              ))}
            </div>
          </div>
